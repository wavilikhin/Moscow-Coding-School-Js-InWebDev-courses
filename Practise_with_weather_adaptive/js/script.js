window.addEventListener("load", function() {
  ...
  function GetWeather(
    ...
    headingSelector,
    tempSelector,
    tempDiagrammSelector,
    pressureSelector,
    pressureDiagrammSelector,
    windSelector,
    windDiagrammSelector
  ) {
    const url =
    ...
    request.addEventListener("readystatechange", function() {
      if (request.status === 200 && request.readyState === 4) {
        ...
        document.querySelector(tempSelector).innerHTML =
          "Temperature:  " + Math.round(Temp) + "°C";
        document.querySelector(tempDiagrammSelector).style.width =
          (Temp + 40) * 2 + "px";
        document.querySelector(pressureSelector).innerHTML =
          "Pressure:  " + Math.round(Pressure * 0.75) + " mmHg";
        document.querySelector(pressureDiagrammSelector).style.width =
          Pressure / 9 + "px";
        document.querySelector(windSelector).innerHTML =
          "Wind Speed:  " + Math.round(Wind) + " m/sec";
        document.querySelector(windDiagrammSelector).style.width =
          Wind * 20 + "px";

        if (Temp < 0) {
          document.querySelector(tempDiagrammSelector).style.backgroundColor =
            "rgb(153, 204, 255)";
        } else if (Temp < 20) {
          document.querySelector(tempDiagrammSelector).style.backgroundColor =
            "rgb(255, 204, 102)";
  ...
  GetWeather(
    ...
    "#MoscowHeader",
    "#MoscowTemp",
    "#MoscowTemperatureDiagramm",
    "#MoscowPressure",
    "#MoscowPressureDiagramm",
    "#MoscowWind",
    "#MoscowWindDiagramm"
  );
  GetWeather(
    ...
    "#LondonHeader",
    "#LondonTemp",
    "#LondonTemperatureDiagramm",
    "#LondonPressure",
    "#LondonPressureDiagramm",
    "#LondonWind",
    "#LondonWindDiagramm"
  );
  GetWeather(
    ...
    "#BerlinHeader",
    "#BerlinTemp",
    "#BerlinTemperatureDiagramm",
    "#BerlinPressure",
    "#BerlinPressureDiagramm",
    "#BerlinWind",
    "#BerlinWindDiagramm"
  );
  GetWeather(
    ...
    "#PekinHeader",
    "#PekinTemp",
    "#PekinTemperatureDiagramm",
    "#PekinPressure",
    "#PekinPressureDiagramm",
    "#PekinWind",
    "#PekinWindDiagramm"
  );
  ...
  addButton.addEventListener("click", function() {
    ...
    GetWeather(
      ...
      "#inputCity" + i + "Header",
      "#inputCity" + i + "Temp",
      "#inputCity" + i + "TemperatureDiagramm",
      "#inputCity" + i + "Pressure",
      "#inputCity" + i + "PressureDiagramm",
      "#inputCity" + i + "Wind",
      "#inputCity" + i + "WindDiagramm"
    );
    if (i == 1) {
...
const desktopBtn = document.querySelector(".desktopCloseBtn");
...
if (document.documentElement.clientWidth < 1025) {
  ...
  function toggleSearch() {
    if (search == "hidden") {
      ...
      plusSect.classList.add("shown");
      searchBlock.classList.add("shown");
      searchBlock.style.display = "block";
      search = "show";
    } else {
      plusSect.classList.remove("shown");
      searchBlock.classList.remove("shown");
...
// BUTTON (desktop) //
if (document.documentElement.clientWidth > 1025) {
  plusBtn.addEventListener("click", showCitySearch);
  let search = "hidden";
  ...
  desktopBtn.addEventListener("click", hideCitySearch);
